...
- **Let's Encrypt** runs Certbot on the Zimbra host with the HTTP-01 standalone
  ...
  and ensure public TCP port 80 is reachable. If Zimbra Proxy occupies local port
  80, the agent stops it for the challenge and always attempts to start it again
  before continuing. The ACME private key is generated and retained on the Zimbra
  host. RSA-2048 is requested for compatibility with Zimbra releases that do not
...
The API starts deployment as a background job and the browser polls its status, so
long certificate operations do not depend on a single proxy request timeout. The
operation can briefly interrupt mail and web access. Premium deployment requires
NetMan agent 0.7.0 or newer; Let's Encrypt requires 0.7.1 for automatic Zimbra
Proxy handling. The first release supports a single Zimbra host; a multi-node
Zimbra deployment must be updated one node at a time.
