<template>
  <div class="animate-fade-in max-w-3xl mx-auto">
    ...
    <div class="bg-base-100 border border-base-300 rounded-none p-6">
      ...
      <h1 class="type-card-title mb-6">Edit MikroTik Router</h1>

      <div v-if="pending" class="flex items-center justify-center py-16">
        <span class="loading loading-spinner loading-lg text-primary"></span>
      </div>

      <form v-else @submit.prevent="updateDevice">
        <div class="space-y-4">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="form-control">
              ...
              <input v-model="deviceForm.password" type="password" class="input input-bordered w-full" placeholder="Leave blank to keep current" />
            </div>
          </div>
          <div class="form-control">
...
const { data: device, pending } = await useFetch(`/api/mikrotik/${deviceId}`)
...
watchEffect(() => {
  if (device.value) {
    const d = device.value as any
    if (!deviceForm.name) {
      deviceForm.name = d.name
      deviceForm.host = d.host
      deviceForm.port = d.port
      deviceForm.username = d.username
      deviceForm.apiVersion = d.apiVersion
      deviceForm.siteId = d.siteId || ''
    }
  }
})
...
async function updateDevice() {
  ...
  try {
    const body: Record<string, unknown> = {
      ...
      apiVersion: deviceForm.apiVersion,
      siteId: deviceForm.siteId || null,
      testConnection: deviceForm.testConnection,
    }
    // Only include password if it was changed
    if (deviceForm.password) {
      body.password = deviceForm.password
    }
    await $fetch(`/api/mikrotik/${deviceId}`, {
      method: 'PUT',
      body,
    })
    ...
    errorMessage.value = err.data?.statusMessage || 'An error occurred while updating the router'
  } finally {
    saving.value = false
  }
}
