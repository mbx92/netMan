<template>
  <div class="animate-fade-in max-w-3xl mx-auto">
    ...
    <div v-else-if="!deviceType" class="bg-base-100 border border-base-300 rounded-none p-6">
      <p class="page-kicker mb-2">Settings</p>
      <h1 class="type-card-title mb-2">Device type not found</h1>
      <p class="type-body-sm text-base-content/60 mb-6">This device type may have been deleted.</p>
      <NuxtLink to="/settings/device-types" class="btn btn-primary">Back to Device Types</NuxtLink>
    </div>

    <div v-else class="bg-base-100 border border-base-300 rounded-none p-6">
      <p class="page-kicker mb-2">Settings</p>
      <h1 class="type-card-title mb-6">Edit Device Type</h1>

      <form @submit.prevent="saveType">
        <div class="space-y-4">
          <div class="form-control">
            <label class="label"><span class="label-text">Code</span></label>
            <input :value="deviceType.code" type="text" class="input input-bordered w-full" disabled />
            <label class="label"><span class="label-text-alt">Code cannot be changed</span></label>
          </div>
          <div class="form-control">
...
const { data: deviceTypesData, pending } = await useFetch<DeviceType[]>('/api/device-types', {
  query: { includeInactive: 'true' }
})

const deviceType = computed(() => {
  return (deviceTypesData.value || []).find(dt => dt.id === id) || null
})
...
async function saveType() {
  saving.value = true
  try {
    await $fetch(`/api/device-types/${id}`, {
      method: 'PUT',
      body: {
        name: formData.name,
